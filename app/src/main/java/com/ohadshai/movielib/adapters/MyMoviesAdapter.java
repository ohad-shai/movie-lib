...
/**
 * Represents the "My Movies" adapter for RecyclerView.
...
public class MyMoviesAdapter extends RecyclerView.Adapter<MyMoviesAdapter.ViewHolder> {
    ...
    /**
     * Initializes a new instance of the "My Movies" adapter for RecyclerView.
     *
    ...
    class ViewHolder extends RecyclerView.ViewHolder implements SelectionHelper.SelectionItemController<Movie> {
        ...
        ViewHolder(final View itemView) {
            ...
            imgbtnMovieWatched.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    if (movie.isWatched()) {
                        imgbtnMovieWatched.setColorFilter(Color.rgb(136, 136, 136));
                        movie.setWatched(false);
                        _repository.movies.updateFolderRelated(movie);
                    } else {
                        imgbtnMovieWatched.setColorFilter(Utils.Colors.WATCHED);
                        movie.setWatched(true);
                        _repository.movies.updateFolderRelated(movie);
                    }
                }
            });
            imgbtnMovieWatched.setOnLongClickListener(new View.OnLongClickListener() {
                @Override
                public boolean onLongClick(View v) {
                    Utils.UI.showInformationToast(imgbtnMovieWatched, R.string.movie_watched);
                    return true;
                }
            });
        ...
        void bindViewHolder(Movie movie) {
            ...
            ratingUser.setRating(movie.displayRating());
            if (movie.isFavorite()) {
                imgbtnMovieFavorite.setColorFilter(Utils.Colors.FAVORITE);
            } else {
                imgbtnMovieFavorite.setColorFilter(Color.rgb(136, 136, 136));
