...
import com.ohadshai.movielib.adapters.MyActorsAdapter;
import com.ohadshai.movielib.db.DBHandler;
import com.ohadshai.movielib.entities.Person;
import com.ohadshai.movielib.loaders.MyActorsLoader;
...
/**
 * A simple {@link Fragment} subclass for "My Actors".
 */
public class MyActorsFragment extends Fragment implements SharedPreferences.OnSharedPreferenceChangeListener, SelectionHelper.SelectionController<Person>, MenuItem.OnMenuItemClickListener {

    //region Constants

    /**
     * Holds a constant for the {@link com.ohadshai.movielib.loaders.MyActorsLoader} loader id.
     */
    private static final int MY_ACTORS_LOADER_ID = 173202;
    ...
    /**
     * Holds the raw list of actors (the list without any manipulations like sort or filters).
     */
    ...
    /**
     * Holds the adapter of the actors list for the {@link RecyclerView}.
     */
    private MyActorsAdapter _adapter;

    /**
     * Holds the {@link RecyclerView} control of the actors list.
     */
    private RecyclerView _rvActors;
    ...
    /**
     * Holds the {@link Button} control to show for empty actors list.
     */
    private Button _btnActorsNotFound;
    ...
    /**
     * Initializes a new instance of simple {@link Fragment} subclass for "My Actors".
     */
    public MyActorsFragment() {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        _view = inflater.inflate(R.layout.fragment_my_actors, container, false);

        getActivity().setTitle(R.string.actors_fragment_title);
        ...
        _rvActors = (RecyclerView) _view.findViewById(R.id.rvCast);
        _rvActors.setLayoutManager(new LinearLayoutManager(getActivity()));
        _adapter = new MyActorsAdapter(_people, this);
        _rvActors.setAdapter(_adapter);

        _btnActorsNotFound = (Button) _view.findViewById(R.id.btnActorsNotFound);
        _btnActorsNotFound.setOnClickListener(new View.OnClickListener() {
            @Override
        ...
        _revealSearch = RevealHelper.create(getActivity(), flSearchContainer, "_revealSearch_MyActors", new RevealHelper.RevealHelperCallback() {
            @Override
        ...
        _revealSelection = RevealHelper.create(getActivity(), _toolbarSelection, "_revealSelection_MyActors", new RevealHelper.RevealHelperCallback() {
            @Override
            public void onReveal() {
            ...
            public void initLayoutControls() {
                ...
                actionShare.setOnMenuItemClickListener(MyActorsFragment.this);

                MenuItem actionDelete = _toolbarSelection.getMenu().findItem(R.id.actionDelete);
                actionDelete.setOnMenuItemClickListener(MyActorsFragment.this);

                MenuItem actionSelectAll = _toolbarSelection.getMenu().findItem(R.id.actionSelectAll);
                actionSelectAll.setOnMenuItemClickListener(MyActorsFragment.this);
    ...
    public void onSharedPreferenceChanged(SharedPreferences sharedPreferences, String key) {
        // Checks if the changed preference is "My Actors Deleted":
        if (key.equals(UIConsts.Preferences.Keys.MY_ACTORS_DELETED) && sharedPreferences.getBoolean(UIConsts.Preferences.Keys.MY_ACTORS_DELETED, false)) {
            _people.clear();
            _peopleRaw.clear();
            _adapter.notifyDataSetChanged();
    ...
    public void onViewStateRestored(@Nullable Bundle savedInstanceState) {
        ...
        // Restores the actors list:
        _peopleRaw = savedInstanceState.getParcelableArrayList(UIConsts.Bundles.PEOPLE_RAW_LIST_KEY);
        ArrayList<Person> actors = savedInstanceState.getParcelableArrayList(UIConsts.Bundles.PEOPLE_LIST_KEY);
        if (actors != null) {
            _people.clear();
    ...
    public void displayListState() {
        ...
        if (this._people.size() < 1) {
            _rvActors.setVisibility(View.GONE);
            _btnActorsNotFound.setVisibility(View.VISIBLE);
        } else {
            _btnActorsNotFound.setVisibility(View.GONE);
            _rvActors.setVisibility(View.VISIBLE);
        }
    }

    /**
     * Loads the "My Actors" list from the DB.
     */
    public void loadMyActorsList() {
        getLoaderManager().initLoader(MY_ACTORS_LOADER_ID, null, new LoaderManager.LoaderCallbacks<ArrayList<Person>>() {
            @Override
            public Loader<ArrayList<Person>> onCreateLoader(int id, Bundle args) {
                return new MyActorsLoader(getContext());
            }
            ...
            public void onLoadFinished(Loader<ArrayList<Person>> loader, ArrayList<Person> data) {
                ...
                _people.clear();
                for (Person actor : data)
                    _people.add(actor);
                _peopleRaw = new ArrayList<>(_people);
                filter(SortAndFilterDialog.getFiltersFromPreferences(_preferences, UIConsts.Keys.SORT_MY_ACTORS_KEY));
                sort(SortAndFilterDialog.getSortFromPreferences(_preferences, UIConsts.Keys.SORT_MY_ACTORS_KEY, _sorts, DEFAULT_SORT_INDEX),
                        SortAndFilterDialog.getSortOrderFromPreferences(_preferences, UIConsts.Keys.SORT_MY_ACTORS_KEY, DEFAULT_SORT_ORDER_IS_ASC));
                _adapter.notifyDataSetChanged();
                displayListState();
            }
    ...
    private void actionSortAndFilter() {
        SortAndFilterDialog.show(getActivity().getFragmentManager(), UIConsts.Keys.SORT_MY_ACTORS_KEY, _sorts, DEFAULT_SORT_INDEX, DEFAULT_SORT_ORDER_IS_ASC, _filters, new SortAndFilterDialog.ResultListener() {
            @Override
    ...
    private void filter(ArrayList<String> selectedFilters) {
        ...
        if (_people.size() > 0) {
            ...
            Snackbar snackbar = Snackbar.make(getActivity().findViewById(R.id.coordinator), R.string.general_msg_no_actors_answer_filter, Snackbar.LENGTH_INDEFINITE)
