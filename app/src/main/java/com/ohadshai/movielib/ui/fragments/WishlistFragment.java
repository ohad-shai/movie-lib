...
import com.ohadshai.movielib.adapters.WishlistAdapter;
import com.ohadshai.movielib.db.DBHandler;
import com.ohadshai.movielib.entities.Movie;
import com.ohadshai.movielib.loaders.WishlistLoader;
...
/**
 * A simple {@link Fragment} subclass for "Wishlist".
 */
public class WishlistFragment extends Fragment implements SharedPreferences.OnSharedPreferenceChangeListener, SelectionHelper.SelectionController<Movie>, MenuItem.OnMenuItemClickListener {

    //region Constants

    /**
     * Holds a constant for the {@link com.ohadshai.movielib.loaders.WishlistLoader} loader id.
     */
    private static final int WISHLIST_LOADER_ID = 173204;
     ...
     */
    private WishlistAdapter _adapter;
    ...
    /**
     * Holds the {@link Button} control to show for empty wishlist list.
     */
    private Button _btnWishlistNotFound;
    ...
    /**
     * Initializes a new instance of simple {@link Fragment} subclass for "Wishlist".
     */
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        _view = inflater.inflate(R.layout.fragment_wishlist, container, false);

        getActivity().setTitle(R.string.wishlist_fragment_title);
                ...
                AddManualOrSelectDialog.show(getActivity().getFragmentManager(), new AddManualOrSelectDialog.ChoiceResultListener() {
                    ...
                    public void onResult(int choiceResult) {
                        if (choiceResult == AddManualOrSelectDialog.CHOICE_MANUAL) {
                            ...
                            intent.putExtra(UIConsts.Bundles.MOVIE_FOLDER_KEY, Movie.Folders.WISHLIST);
        ...
        _adapter = new WishlistAdapter(_movies, this);
        _rvMovies.setAdapter(_adapter);

        _btnWishlistNotFound = (Button) _view.findViewById(R.id.btnWishlistNotFound);
        _btnWishlistNotFound.setOnClickListener(new View.OnClickListener() {
            @Override
        ...
        _revealSearch = RevealHelper.create(getActivity(), flSearchContainer, "_revealSearch_Wishlist", new RevealHelper.RevealHelperCallback() {
            @Override
        ...
        _revealSelection = RevealHelper.create(getActivity(), _toolbarSelection, "_revealSelection_Wishlist", new RevealHelper.RevealHelperCallback() {
            @Override
            public void onReveal() {
            ...
            public void initLayoutControls() {
                ...
                MenuItem actionEdit = _toolbarSelection.getMenu().findItem(R.id.actionEdit);
                actionEdit.setOnMenuItemClickListener(WishlistFragment.this);

                MenuItem actionShare = _toolbarSelection.getMenu().findItem(R.id.actionShare);
                actionShare.setOnMenuItemClickListener(WishlistFragment.this);

                MenuItem actionDelete = _toolbarSelection.getMenu().findItem(R.id.actionDelete);
                actionDelete.setOnMenuItemClickListener(WishlistFragment.this);

                MenuItem actionSelectAll = _toolbarSelection.getMenu().findItem(R.id.actionSelectAll);
                actionSelectAll.setOnMenuItemClickListener(WishlistFragment.this);
    ...
    public void onSharedPreferenceChanged(SharedPreferences sharedPreferences, String key) {
        // Checks if the changed preference is "Wishlist Deleted":
        if (key.equals(UIConsts.Preferences.Keys.WISHLIST_DELETED) && sharedPreferences.getBoolean(UIConsts.Preferences.Keys.WISHLIST_DELETED, false)) {
            _movies.clear();
            _moviesRaw.clear();
            _adapter.notifyDataSetChanged();
    ...
    public void displayListState() {
        ...
        if (this._movies.size() < 1) {
            ...
            _btnWishlistNotFound.setVisibility(View.VISIBLE);
        } else {
            _btnWishlistNotFound.setVisibility(View.GONE);
            _rvMovies.setVisibility(View.VISIBLE);
        }
    }

    /**
     * Loads the "Wishlist" list from the DB.
     */
    public void loadWishlistList() {
        getLoaderManager().initLoader(WISHLIST_LOADER_ID, null, new LoaderManager.LoaderCallbacks<ArrayList<Movie>>() {
            @Override
            public Loader<ArrayList<Movie>> onCreateLoader(int id, Bundle args) {
                return new WishlistLoader(getContext());
            }
            ...
            public void onLoadFinished(Loader<ArrayList<Movie>> loader, ArrayList<Movie> data) {
                ...
                filter(SortAndFilterDialog.getFiltersFromPreferences(_preferences, UIConsts.Keys.SORT_WISHLIST_KEY));
                sort(SortAndFilterDialog.getSortFromPreferences(_preferences, UIConsts.Keys.SORT_WISHLIST_KEY, _sorts, DEFAULT_SORT_INDEX),
                        SortAndFilterDialog.getSortOrderFromPreferences(_preferences, UIConsts.Keys.SORT_WISHLIST_KEY, DEFAULT_SORT_ORDER_IS_ASC));
                _adapter.notifyDataSetChanged();
                displayListState();
            }
    ...
    private void actionSortAndFilter() {
        SortAndFilterDialog.show(getActivity().getFragmentManager(), UIConsts.Keys.SORT_WISHLIST_KEY, _sorts, DEFAULT_SORT_INDEX, DEFAULT_SORT_ORDER_IS_ASC, _filters, new SortAndFilterDialog.ResultListener() {
            @Override
